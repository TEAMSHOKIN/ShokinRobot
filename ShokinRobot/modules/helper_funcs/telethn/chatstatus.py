...
from ShokinRobot import DRAGONS
from ShokinRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Shokin_is_admin(chat_id: int):
    status = False
    Shokin = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Shokin.id == user.id:
            status = True
            break
    return status
