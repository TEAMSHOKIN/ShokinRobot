...
import ShokinRobot.modules.sql.users_sql as sql
from ShokinRobot import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
    OWNER_ID,
...
from ShokinRobot.modules import ALL_MODULES
from ShokinRobot.modules.helper_funcs.chat_status import is_user_admin
from ShokinRobot.modules.helper_funcs.misc import paginate_modules
...
buttons = [
    ...
    [
        ...
        InlineKeyboardButton(text="🏡Oғғɪᴄᴇ🏡", url=f"https://t.me/ShokinXBots"),
    ],
    [
        InlineKeyboardButton(text="🌿Oᴡɴᴇʀ🌿", url=f"https://t.me/Shokin_WAS_BOT"),
    ],
]
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("ShokinRobot.modules." + module_name)
...
@run_async
def Shokin_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "Shokin_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="📍𝐎𝐖𝐍𝐄𝐑📍", url=f"https://t.me/Shokin_WAS_BOT"
                        ),
                        InlineKeyboardButton(
                            text="🎄𝐑𝐄𝐏𝐎🎄", 
                            url="https://t.me/Shokin_WAS_BOT",
                        ),
                    ],
                    [
                        InlineKeyboardButton(text="✯ вαϲк ✯", callback_data="Shokin_back"),
                    ],
                ]
            ),
        )
    elif query.data == "Shokin_back":
        first_name = update.effective_user.first_name 
        query.message.edit_text(
            PM_START_TEXT.format(escape_markdown(first_name), (START_IMG), BOT_NAME),
            reply_markup=InlineKeyboardMarkup(buttons),
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != {OWNER_ID} and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [𝙰𝙻𝙾𝙽𝙴](https://t.me/Shokin_WAS_BOT)"
...
♨️𝗠𝗔𝗗𝗘 𝗕𝗬 [𝗔𝗟𝗢𝗡𝗘](https://t.me/Shokin_WAS_BOT)♨️
...
""",
    ...
    about_callback_handler = CallbackQueryHandler(
        Shokin_about_callback, pattern=r"Shokin_"
    )
